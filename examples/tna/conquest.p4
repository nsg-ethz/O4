...
control SwitchEgress(
    ...
    action calc_hashed_index_TCP(){
           eg_md.hashed_index_row_0 = hash_0_TCP.get({
               5w8, hdr.ipv4.src_addr,
               5w12, hdr.ipv4.dst_addr,
               4w10, hdr.tcp.src_port,
               6w28, hdr.tcp.dst_port
           });
           eg_md.hashed_index_row_1 = hash_1_TCP.get({
               4w11, hdr.ipv4.src_addr,
               6w22, hdr.ipv4.dst_addr,
               3w0, hdr.tcp.src_port,
               3w4, hdr.tcp.dst_port
           });
    }
    action calc_hashed_index_UDP(){
           eg_md.hashed_index_row_0 = hash_0_UDP.get({
               3w6, hdr.ipv4.src_addr,
               5w20, hdr.ipv4.dst_addr,
               3w6, hdr.udp.src_port,
               3w6, hdr.udp.dst_port
           });
           eg_md.hashed_index_row_1 = hash_1_UDP.get({
               6w31, hdr.ipv4.src_addr,
               3w2, hdr.ipv4.dst_addr,
               6w24, hdr.udp.src_port,
               3w4, hdr.udp.dst_port
           });
    }
    action calc_hashed_index_Other(){
           eg_md.hashed_index_row_0 = hash_0_Other.get({
               6w3, hdr.ipv4.src_addr,
               6w24, hdr.ipv4.dst_addr,
               4w14, hdr.ipv4.protocol
           });
           eg_md.hashed_index_row_1 = hash_1_Other.get({
               4w3, hdr.ipv4.src_addr,
               6w31, hdr.ipv4.dst_addr,
               5w14, hdr.ipv4.protocol
           });
    }
